# Gate 4 Checklist: Stage 2 Artifacts Batch Sign-off

## Purpose
Gate 4 在 Stage 2 所有并行组完成后、Stage 3 代码生成前执行。批量审阅 4 个产出物：UI Prototype、Frontend Architecture、Backend Architecture、Test Cases。

**Gate 3 已完成 PRD 单独验证，本 Gate 不再重复检查 PRD。**

## Parallel Group Completion Check
在开始自动验证前，确认所有并行组已完成：
- [ ] Group A (prototype + backend_arch) 状态为 completed
- [ ] Group B (frontend_arch) 状态 completed
...
### 1. UI Prototype (对照 prototype-schema.yaml)
- [ ] `docs/03-prototype/index.html` 存在且为有效 HTML
- [ ] 页面清单 ≥1，每个页面有 name/path/description/primary_actions
- [ ] 导航结构完整，引用的页面文件都存在
- [ ] 链接/按钮可交互（有 href 或 onclick）
- [ ] 覆盖 requirement 的所有核心场景
- [ ] 主要流程无 lorem-ipsum 占位

### 2. Frontend Architecture (根据 tech_stack.frontend 选择 schema)
...
### 3. Backend Architecture (根据 tech_stack.backend 选择 schema)
...
### 4. Test Cases (对照 test-case-schema.yaml)
...
- [ ] PRD 已确认（gate3 已通过，不在本 Gate 重复检查）
